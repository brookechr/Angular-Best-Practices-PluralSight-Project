...
//putting an empty line between imports from angular and imports from your project is a best practice

@Injectable()
export class CatalogRepositoryService {

  constructor(private userRespository: UserRepositoryService) {}

  getCatalog():Observable<any[]> {
    const subject = new Subject<any>();
    const currentUser = this.userRespository.currentUser || {classes:[]};
...
//don't need all upper case constant name bc ide tells us that we can't rename constants, so we don't need a visual indicator
const courses = [{
  courseNumber: 'PO101',
